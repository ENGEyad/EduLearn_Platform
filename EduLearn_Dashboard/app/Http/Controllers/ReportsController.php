...
class ReportsController extends Controller
{
    ...
    public function list(Request $request)
    {
        ...
        // Search logic: matches class name OR any student in the class
        if ($search !== '') {
            $query->where(function ($q) use ($search) {
                $searchTerm = "%{$search}%";

                // Handle concatenation differently based on database driver
                $driver = DB::connection()->getDriverName();
                if ($driver === 'sqlite') {
                    $q->where(DB::raw("grade || ' - ' || class_section"), 'like', $searchTerm);
                } else {
                    $q->where(DB::raw("CONCAT(grade, ' - ', class_section)"), 'like', $searchTerm);
                }

                // OR if any student in this group matches the search
        ...
        $items = $query->orderBy('grade')->orderBy('class_section')
            ->get()
            ->map(function ($item) {
                $item->students_count = (int) $item->students_count;
                return $item;
            });

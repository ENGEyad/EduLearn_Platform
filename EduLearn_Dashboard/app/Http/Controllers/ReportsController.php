...
class ReportsController extends Controller
{
    ...
    public function list(Request $request)
    {
        ...
        // Optimization: Use a single GROUP BY query with count to avoid N+1 queries.
        // This reduces queries from O(N) where N is number of classes to O(1).
        $query = Student::select('grade', 'class_section', DB::raw('count(*) as students_count'))
            ->whereNotNull('grade')
            ->whereNotNull('class_section');

        // If filtering by subject, finding the class matching the subject Filter
        if ($subjectFilter !== '') {
        ...
        // Search logic moved to database level for better performance
        if ($search !== '') {
            $query->where(function ($q) use ($search) {
                // Determine driver for string concatenation
        ...
        $items = $query->groupBy('grade', 'class_section')
            ...
            ->get()
            ->map(function ($item) {
                // Ensure student_count is an integer
                return [
                    'grade' => $item->grade,

...
class ReportsController extends Controller
{
    ...
    /**
     * Get a list of classes (grade + section) with student counts.
    ...
    public function list(Request $request)
    {
        ...
        // Build a single query to get classes and their student counts
        $query = Student::query()
        ...
        // Apply filters directly to the grouped query
        if ($subjectFilter !== '') {
        ...
        if ($search !== '') {
            $query->where(function ($q) use ($search) {
                // Check if class name matches search (e.g., "Grade 1 - A")
                // Using a concat for the search on grouped class name.
                // We use DB::connection()->getDriverName() to handle both SQLite and MySQL concatenation.
                $concat = DB::connection()->getDriverName() === 'sqlite'
                    ? "grade || ' - ' || class_section"
                    : "CONCAT(grade, ' - ', class_section)";

                $q->where(DB::raw($concat), 'like', "%{$search}%")
                  // OR if any student in that class matches search

...
class ReportsController extends Controller
{
    ...
    /**
     * Optimized Reports list endpoint.
     * Replaces N+1 query loop with a single grouped query and server-side search.
     * ⚡ Bolt Optimization: Reduced database hits from N+1 to a constant number of queries.
     */
    public function list(Request $request)
    {
        ...
        // Start with a grouped query to get student counts per class/section
        $query = Student::select('grade', 'class_section', DB::raw('count(*) as students_count'))
            ->whereNotNull('grade')
            ->whereNotNull('class_section')
        ...
        // Handle search server-side to avoid fetching all classes and looping in memory
        if ($search !== '') {
            $query->where(function ($q) use ($search) {
                // Determine driver for concatenation syntax
                $driver = DB::connection()->getDriverName();
                $concat = $driver === 'sqlite'
                    ? "grade || ' - ' || class_section"
                    : "CONCAT(grade, ' - ', class_section)";

                $q->whereRaw("LOWER($concat) LIKE ?", ["%" . strtolower($search) . "%"])
                  ->orWhereExists(function ($subQuery) use ($search) {
                      $subQuery->select(DB::raw(1))
                          ...
                          ->where(function ($sq) use ($search) {
                              $sq->where('s2.full_name', 'like', "%{$search}%")
        ...
        // Convert string counts to integers (MySQL/SQLite sometimes return them as strings)
        ...
        // Separate query for matching students if search is active
        $matchingStudents = collect();
        if ($search !== '') {
            $studentSearchQuery = Student::where(function ($q) use ($search) {
            ...
            if ($classFilter !== '') {
                $parts = explode('-', $classFilter);
                if (count($parts) == 2) {
                    $studentSearchQuery->where('grade', trim($parts[0]))
                        ->where('class_section', trim($parts[1]));
                }
            }

            $matchingStudents = $studentSearchQuery->get(['id', 'full_name', 'academic_id', 'grade', 'class_section', 'photo_path']);
        }

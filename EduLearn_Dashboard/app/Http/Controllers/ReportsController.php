...
class ReportsController extends Controller
{
    ...
    public function list(Request $request)
    {
        ...
        // Start with a base query of grouped classes and counts
        $query = Student::select('grade', 'class_section', DB::raw('count(*) as students_count'))
            ->whereNotNull('grade')
            ->whereNotNull('class_section')
            ->groupBy('grade', 'class_section');

        // If filtering by subject, finding the class matching the subject Filter in DB
        if ($subjectFilter !== '') {
        ...
        // Apply search filter directly in SQL
        if ($search !== '') {
            $query->where(function ($q) use ($search) {
                ...
                // Or if any student in this class matches

...
class ReportsController extends Controller
{
    ...
    // Optimized: Uses a single grouped query to avoid N+1 issues
    public function list(Request $request)
    {
        ...
        // Start with a base query that groups students by grade and section
        ...
        // Handle search: matches class name (concatenated) or any student in the class
        if ($search !== '') {
            $driver = DB::connection()->getDriverName();
            // String concatenation is different in SQLite (||) vs MySQL (CONCAT)
            $concat = ($driver === 'sqlite') ? "grade || ' - ' || class_section" : "CONCAT(grade, ' - ', class_section)";

            $query->where(function ($q) use ($search, $concat) {
                $q->whereRaw("{$concat} LIKE ?", ["%{$search}%"])
                    ->orWhereExists(function ($sub) use ($search) {
                        $sub->select(DB::raw(1))
                            ->from('students as s2')
                            ->whereColumn('s2.grade', 'students.grade')
                            ->whereColumn('s2.class_section', 'students.class_section')
                            ->where(function ($subSearch) use ($search) {
                                $subSearch->where('full_name', 'like', "%{$search}%")
        ...
        // If search is active, we also grab matching students directly as before
        $matchingStudents = collect();
        if ($search !== '') {
            $studentSearchQuery = Student::query();
            ...
            $matchingStudents = $studentSearchQuery->where(function ($q) use ($search) {
                $q->where('full_name', 'like', "%{$search}%")
                    ->orWhere('academic_id', 'like', "%{$search}%");
            })->get(['id', 'full_name', 'academic_id', 'grade', 'class_section', 'photo_path']);
        }

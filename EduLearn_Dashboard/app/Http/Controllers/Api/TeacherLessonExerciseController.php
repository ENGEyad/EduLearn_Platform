...
class TeacherLessonExerciseController extends Controller
{
    ...
    public function publish(Request $request, int $lesson): JsonResponse
    {
        ...
        // Record activity for students
        $students = \App\Models\Student::where('class_section_id', $lessonModel->class_section_id)->get();
        foreach ($students as $student) {
            $this->activityService->recordTeacherExerciseActivityForStudent([
                'teacher_id' => $teacher->id,
                'teacher_code' => $teacher->teacher_code,
                'teacher_name' => $teacher->full_name,
                'student_id' => $student->id,
                'academic_id' => $student->academic_id,
                'class_section_id' => $lessonModel->class_section_id,
                'subject_id' => $lessonModel->subject_id,
                'lesson_id' => $lessonModel->id,
                'exercise_set_id' => $set->id,
                'event_type' => \App\Models\LearningActivity::EVENT_TEACHER_PUBLISHED_EXERCISE,
                'title' => 'تم نشر تمارين جديدة',
                'body' => "قام الأستاذ {$teacher->full_name} بنشر تمارين للدرس: {$lessonModel->title}",
            ]);
        }

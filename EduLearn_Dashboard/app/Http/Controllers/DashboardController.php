...
class DashboardController extends Controller
{
    public function index(Request $request)
    {
        ...
        // ⚡ Bolt Optimization: Cache AI response to reduce redundant slow API calls.
        // The cache key is unique to the period and the data snapshot (md5 of summary).
        $cacheKey = "ai_insight_{$period}_" . md5($statsSummary);

        $cachedResult = Cache::get($cacheKey);
        if ($cachedResult) {
            $aiInsight = $cachedResult;
        } else {
            try {
                ...
                if ($response->successful()) {
                    ...
                    // Cache only successful results for 1 hour

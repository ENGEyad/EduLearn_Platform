...
class TeacherSettingsScreen extends StatelessWidget {
  ...
  const TeacherSettingsScreen({Key? key, required this.teacher}) : super(key: key);
        ...
        Row(
          ...
          children: [
            if (teacher.assignments != null)
              _buildStatBadge(context, '${teacher.assignments!.length}', l10n.get('active_classes')),
            if (teacher.assignments != null && teacher.totalAssignedStudents != null)
              const SizedBox(width: 16),
            if (teacher.totalAssignedStudents != null)
              _buildStatBadge(context, '${teacher.totalAssignedStudents}', l10n.get('assigned_students')),
          ],
        )
      ],
    );
  ...
  Widget _buildStatBadge(BuildContext context, String value, String label) {
    return Container(
      ...
      child: Column(
        children: [
          Text(value, style: TextStyle(fontWeight: FontWeight.bold, color: Theme.of(context).colorScheme.onPrimaryContainer)),
          Text(label, style: TextStyle(fontSize: 10, color: Theme.of(context).colorScheme.onPrimaryContainer)),
        ],
      ),
    );
  }
